...
def mfcc_generate(wav_file, length=None):
        ...
        sample_rate , signal_ = scipy.io.wavfile.read(wav_file)

        if length != 'None':
                signal_ = signal_[0:int(3.5 * sample_rate)]

        #scipy.io.wavfile.write('SA1.wav',sample_rate,signal_)
        print len(signal_)
        y = range(len(signal_))
        NFFT = 512
        nfilt = 40
        ...
        mfcc *= lift  #*
        print 'The length of the mfcc is',len(mfcc)
        return mfcc

print mfcc_generate('sam.wav')